...
CopyOnWriteArrayList底层剖析：

CopyOnWriteArrayList底层是实现List的，底层也是object[]数组，被Volatile关键字修饰。添加原理也是和ArrayList一样的底层是Arrays.copyOf内部调用System.arraycopy。
...
解决：ConcurrentHashMap 或 Collections.synchronizedMap 或者
...
```java
package com.haxdeu.interview2.reentrant;
